...
import { editInstitutionInfo } from "../../utills/editLayoutItem";
...
const ContactUpdateModal = ({
  ...
  const {
    ...
    staleTime: Infinity,
    queryKey: ["layout", { id }],
    queryFn: () => getLayoutData(`/layout/${id}`),
  });
  ...
  */
  const { mutateAsync, isLoading, isError, error } = useMutation({
    mutationFn: (value) => editInstitutionInfo(value),
    onSuccess: async () => {
      refetch();
  ...
  */
  const formik = useFormik({
    ...
    initialValues: {
      institution_name: data?.payload[type]?.institution_name || "",
      location_name: data?.payload[type]?.location_name || "",
      institutionCode: data?.payload[type]?.institutionCode || "",
      EIIN: data?.payload[type]?.EIIN || "",
      established: data?.payload[type]?.established || "",
      village: data?.payload[type]?.village || "",
      district: data?.payload[type]?.district || "",
      upazila: data?.payload[type]?.upazila || "",
      postOffice: data?.payload[type].postOffice || "",
      email: data?.payload[type]?.email || "",
      phone: data?.payload[type]?.phone || "",
      website: data?.payload[type]?.website || "",
      map_link: data?.payload[type]?.map_link || "",
      logo: undefined,
    },
    validationSchema,
    onSubmit: async (values) => {
      ...
      await mutateAsync({ url: `/layout/edit/${id}`, data });
    },
  });
